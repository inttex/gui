...
class Camera:
    ...
    def __init__(self, pos=dpg.mvVec4(0.0, 0.0, 0.0, 1.0), pitch=0.0, yaw=0.0):
        ...
        dpg.create_lookat_matrix([self.eye_pos[_] for _ in range(4)], [self.view_to[_] for _ in range(4)], self.eye_up)
    ...
    def view_matrix(self):
        return dpg.create_lookat_matrix([self.eye_pos[_] for _ in range(4)], [self.view_to[_] for _ in range(4)], self.eye_up)
...
class Sphere:
    def __init__(self, pos=(0, 0, 0), radius=1, alpha=255):
        self.radius = radius
        self.alpha = alpha
        ...
        with dpg.draw_node(tag=self.node):
            ...
            for theta in np.linspace(-np.pi / 2, np.pi / 2, nb_thetas, endpoint=False):
                for phi in np.linspace(0, 2 * np.pi, nb_phis, endpoint=False):
                    ...
                    dpg.draw_triangle(pts4[0], pts4[1], pts4[2], color=[255, 255, 255, 10],
                                      fill=[150, 150, 150, self.alpha], thickness=1)
                    dpg.draw_triangle(pts4[2], pts4[3], pts4[0], color=[255, 255, 255, 10],
                                      fill=[150, 150, 150, self.alpha], thickness=1)

        with dpg.draw_node(tag=self.node2):
            theta = 0
            for phi in np.linspace(0, 2 * np.pi, nb_phis, endpoint=False):
                dpg.draw_line(self.getxyz(self.radius, theta, phi), self.getxyz(self.radius, theta, phi + d_phi),
                              color=(255, 0, 0))

            for phi in [0, np.pi]:
                for theta in np.linspace(-np.pi / 2, np.pi / 2, nb_thetas, endpoint=False):
                    dpg.draw_line(self.getxyz(self.radius, theta, phi), self.getxyz(self.radius, theta + d_theta, phi),
                                  color=(0, 255, 0))
...
sphere = Sphere(radius=10, alpha=255)

sphere2 = Sphere(pos=[0, 4, 6, ], radius=5, alpha=255)
